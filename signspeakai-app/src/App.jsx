...
function App() {
  ...
  const URL_MODELO = 'https://teachablemachine.withgoogle.com/models/nYbXwzuLj/';
  ...
  useEffect(() => {
    ...
    async function cargarModeloYCamara() {
      try {
        ...
        // Obtener clases globales cargadas desde el index.html
        ...
        hands.onResults(async (results) => {
          ...
          if (videoRef.current && model) {
            ...
            if (mejorPrediccion.probability > 0.85) {
  ...
  return (
    <div style={{ textAlign: 'center', backgroundColor: '#1e1e2e', color: 'white', minHeight: '100vh', padding: '20px' }}>
      <h1>signSpeakAI — Traductor en Vivo</h1>

      {/* Cajas de Resultado */}
      <div style={{ margin: '20px auto', padding: '15px', backgroundColor: '#2b2b3d', borderRadius: '10px', maxWidth: '500px' }}>
        <h2>Traducción: <span style={{ color: '#00FF00' }}>{traduccion}</span></h2>
      ...
      <div style={{ position: 'relative', display: 'inline-block' }}>
        ...
        <canvas ref={canvasRef} width="640" height="480" style={{ borderRadius: '15px', border: '3px solid #00FF00' }} />
      </div>
    </div>
  );
}
